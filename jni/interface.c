...
int Java_com_topvs_client_NetplayerAPI_initServerInfo(JNIEnv* env,
		jclass object, jstring ip_addr, jint port, jstring user_name,
		jstring userpasword, jstring cu_id, jint proto) {
	LOGI("初始化....");
	int ret = initPlayer();
	if (ret != 0)
		return -1;
...
jint Java_com_topvs_client_NetplayerAPI_deinitServerInfo(JNIEnv* env,
		jclass object, jint user_id) {
	releasePlayer();
	return st_net_deinitServerInfo(user_id);
}

// 设置user id对应ip等配置信息   Date: 2014-3-28 Author: yms
jint Java_com_topvs_client_NetplayerAPI_setServerInfo(JNIEnv* env,
		jclass object, jint user_id, jstring ip_addr, jint port,
		jstring user_name, jstring userpasword) {
	if (0 == user_id)
		return -1;
...
jint Java_com_topvs_client_NetplayerAPI_setVideoConfig(JNIEnv* env, jclass obj,
		jint user_id, jint channel, jbyteArray pOutArray) {
...
jint Java_com_topvs_client_NetplayerAPI_setVideoEncode(JNIEnv* env, jclass obj,
		jint user_id, jint channel, jintArray pOutArray) {
...
jint Java_com_topvs_client_NetplayerAPI_setRemoteIceInfo(JNIEnv* env,
		jclass object, jint user_id, jstring ice_info, jint ice_size) {
	jboolean isCopy = JNI_TRUE;
	const char* iceinfo = (*env)->GetStringUTFChars(env, ice_info, &isCopy);
	return st_net_set_iceinfo(user_id, iceinfo, ice_size);
}

jint Java_com_topvs_client_NetplayerAPI_userLogin(JNIEnv* env, jclass object,
		jint user_id) {
	int ret = st_net_userLogin(user_id);
	return ret;
}

jint Java_com_topvs_client_NetplayerAPI_userLogout(JNIEnv* env, jclass object,
		jint user_id) {
	return st_net_userLogout(user_id, TRUE);
}

//add by LinZh107 2015-6-2
jint Java_com_topvs_client_NetplayerAPI_getAllIOState(JNIEnv* env, jclass cls,
		jint user_id, jobject obj_io) {
	...
	(*env)->SetByteArrayRegion(env, output, 0, MAX_IO_OUTPUT_NUM,
			myiodata.output);
...
jint Java_com_topvs_client_NetplayerAPI_setAllIOState(JNIEnv* env, jclass cls,
		jint user_id, jobject obj_io) {
	//find java class
	jclass io_cls = (*env)->GetObjectClass(env, obj_io);
	if (io_cls == NULL)
...
jint Java_com_topvs_client_NetplayerAPI_setIOState(JNIEnv* env, jclass cls,
		jint user_id, jint swindex, jboolean flags, jint sec) {
	dev_io_control_t iodata;
	memset(&iodata, 0, sizeof(iodata));
	int ret = tp_net_get_io_state(user_id, &iodata);
	if (ret < 0)
	...
	LOGI(
			"iodata.output[%d].status = %0x", swindex, iodata.output[swindex].status);
...
jint Java_com_topvs_client_NetplayerAPI_ptzControl(JNIEnv* env, jclass object,
		jint user_id, jint type, jint channel, jint param) {
...
jint Java_com_topvs_client_NetplayerAPI_openChannelStream(JNIEnv* env,
		jclass object, jint user_id, jint channel, jint protocol_type,
		jint channel_type) {
	LOGI("Java_com_topvs_client_NetplayerAPI_openChannelStream");
	return st_net_openChannelStream(user_id, channel, protocol_type,
			channel_type);
}

jint Java_com_topvs_client_NetplayerAPI_isSurportSlave(JNIEnv* env,
		jclass object, jint user_id) {
 ...
 */
jint Java_com_topvs_client_NetplayerAPI_startChannelStream(JNIEnv* env,
		jclass object, jint user_id, jint channel, jint stream_type,
		jint channel_type) {
	...
	return st_net_startChannelStream(user_id, channel, stream_type,
			channel_type);
}

jint Java_com_topvs_client_NetplayerAPI_closeChannelStream(JNIEnv* env,
		jclass object, jint user_id, jint channel, jint channel_type) {
	return st_net_closeChannelStream(user_id, channel, channel_type);
}

jint Java_com_topvs_client_NetplayerAPI_stopChannelStream(JNIEnv* env,
		jclass object, jint user_id, jint channel, jint stream_type,
		jint channel_type) {
...
jint Java_com_topvs_client_NetplayerAPI_changeStreamType(JNIEnv* env,
		jclass object, jint user_id, jint channel, jint stream_type,
		jint channel_type) {
	int ret = -1;
	switch (stream_type) {
	case 0:
		...
		ret = st_net_stopChannelStream(user_id, channel, stream_type + 1,
				channel_type);
		if (ret != 0)
			return -1;
		LOGI("NetplayerAPI_changeStream_type 2");
		ret = -1;
		ret = st_net_startChannelStream(user_id, channel, stream_type,
				channel_type);
		if (ret != 0)
			return -2;
		break;
...
jint Java_com_topvs_client_NetplayerAPI_defaultVidioParame(JNIEnv* env,
		jclass object, jint user_id, jint channel) {
...
jint Java_com_topvs_client_NetplayerAPI_getVideoConfig(JNIEnv* env, jclass obj,
		jint user_id, jint channel, jbyteArray pOutArray) {
	jbyte *byteArray = (*env)->GetByteArrayElements(env, pOutArray, 0);
...
jint Java_com_topvs_client_NetplayerAPI_getRecordInfo(JNIEnv* env, jclass obj,
		jint user_id,jint channel, jintArray startArr, jintArray endArr,
		jobjectArray RecordName) {
	jint *sArr = (*env)->GetIntArrayElements(env, startArr, 0);
	jint *eArr = (*env)->GetIntArrayElements(env, endArr, 0);
	msg_search_file_t record_param;
	record_param.channel = channel;
	record_param.flag = 0;
	record_param.record_type = 31;
	record_param.s_tm.year = sArr[0];
	record_param.s_tm.month = sArr[1];
	record_param.s_tm.day = sArr[2];
	record_param.s_tm.hour = sArr[3];
	record_param.s_tm.minute = sArr[4];
	record_param.s_tm.second = sArr[5];

	record_param.e_tm.year = eArr[0];
	record_param.e_tm.month = eArr[1];
	record_param.e_tm.day = eArr[2];
	record_param.e_tm.hour = eArr[3];
	record_param.e_tm.minute = eArr[4];
	record_param.e_tm.second = eArr[5];
//	msg_fragment_t p_record_info[1000];
	msg_fragment_t *p_record_info = (msg_fragment_t *)malloc(1000*sizeof(msg_fragment_t));
	memset(p_record_info, 0, 1000*sizeof(msg_fragment_t));
	int ret = st_net_queryFileListByTime(user_id, record_param, p_record_info);
	LOGI("ret=:%d", ret);
	int i;
	jstring str = NULL;
//	net_msg_t
	for (i = 0; i < ret; i++) {
		char name[100]={};
		sprintf(name, "%02d:%02d:%02d-%02d:%02d:%02d", p_record_info[i].start_time.hour,
				p_record_info[i].start_time.minute,
				p_record_info[i].start_time.second,
				p_record_info[i].end_time.hour,
				p_record_info[i].end_time.minute,
				p_record_info[i].end_time.second);

//		LOGI("name%d:%s", ret, name);
		LOGI("recordname:%s",p_record_info[i].file_name);
		str = (*env)->NewStringUTF(env, name);
		(*env)->SetObjectArrayElement(env, RecordName, i, str);
...
jint Java_com_topvs_client_NetplayerAPI_startRecordPlayer(JNIEnv* env, jclass obj,
		jint user_id,jint index){
//	st_destroyFrameList();
	st_initFrameList();
//	channel_info_t
	int remote_play_id = st_net_startrecord(user_id,index);
//	remote_play_t
//	PLAY_INSTANCE *pInstance = (PLAY_INSTANCE *)player_id;
//		if (pInstance == NULL) return FALSE;
//
//		CPlaySDK *pSDK = (CPlaySDK *)pInstance->lpSDKObject;
//		if (pSDK == NULL) return FALSE;
	return remote_play_id;
}

jint Java_com_topvs_client_NetplayerAPI_stopRecordPlayer(JNIEnv* env, jclass obj,
		jint play_id){
	int ret = st_net_stopRemotePlayByTime(play_id);
	st_destroyFrameList();
	return ret;
}

jint Java_com_topvs_client_NetplayerAPI_downLoadRecord(JNIEnv* env, jclass obj,
		jint user_id,jint index,jstring file)
{
	//server_info_t
	jboolean iscopy = JNI_TRUE;
	const char *filename = (*env)->GetStringUTFChars(env, file, &iscopy);
	LOGI("呵呵");
	int ret = st_net_downloadRecord(user_id,index,filename);
	return ret;
}

jint Java_com_topvs_client_NetplayerAPI_cancleDownload(JNIEnv* env, jclass obj,
		jint user_id)
{
	int ret = st_net_stopdownrecord(user_id);
	return ret;
}
jint Java_com_topvs_client_NetplayerAPI_deleteServerRecord(JNIEnv* env, jclass obj,
		jint user_id,jint index)
{
	int ret = st_net_deleteRecord(user_id,index);
	return ret;
}
jint Java_com_topvs_client_NetplayerAPI_getDownloadProgress(JNIEnv* env, jclass obj,
		jobjectArray progress)
{
	LOGI("111");
	msg_file_down_Status  *down_progress = (msg_file_down_Status *)malloc(sizeof(msg_file_down_Status));
	memset(down_progress,0,sizeof(msg_file_down_Status));
	st_net_getprogress(down_progress);
	LOGI("222");
	int current = down_progress[0].iFileSize - down_progress[0].iTransfersSize;
	LOGI("current:%d__all:%d",current, down_progress[0].iFileSize);
	char  currents[100] = {};
	sprintf(currents,"%d",current);
	char total[100] = {};
	sprintf(total,"%d",down_progress[0].iFileSize);
	LOGI("333");
	jstring str_c = NULL;
	jstring str_t = NULL;
	str_c = (*env)->NewStringUTF(env, currents);
	str_t = (*env)->NewStringUTF(env, total);
	(*env)->SetObjectArrayElement(env, progress, 0, str_c);
	(*env)->SetObjectArrayElement(env, progress, 1, str_t);
	LOGI("444");
	return 0;
}

jint Java_com_topvs_client_NetplayerAPI_getVideoEncode(JNIEnv* env, jclass obj,
		jint user_id, jint channel, jint stream_type, jintArray pOutArray) {
	jint *intArray = (*env)->GetIntArrayElements(env, pOutArray, 0);

	video_encode_t encode;
	int ret = st_net_getVideoEncodeParam(user_id, channel, &encode,
			stream_type);
...
jint Java_com_topvs_client_NetplayerAPI_getChannelNum(JNIEnv* env,
		jclass object, jint user_id) {
...
jint Java_com_topvs_client_NetplayerAPI_getVideoFrame(JNIEnv* env, jclass jobj,
		jintArray pOutArray, jobject bitmap) {
//	LOGI("走了");
...
//	LOGE("堵死了");
	...
	int ret = decodeH264video(pVideoFrame->data, pVideoFrame->frame_size,
			pVideoFrame->frame_rate, intArray, g_oneFrameBuf);
	if (ret == 0) {
	...
	error: (*env)->ReleaseIntArrayElements(env, pOutArray, intArray, 0);
	st_removeFrameListHead(VIDEO_FRAME);
	return -2;
}
...
// 更换报警类型   Date: 2014-5-22 Author: LinZh107
jint Java_com_topvs_client_NetplayerAPI_GetdevInfo(JNIEnv* env, jclass jobj,
		jint channel, jobject obj_devInfo, jobject obj_alarmTime) {
	//standard C
	...
	if (0 == devInfoCls)
		return -1;
	jfieldID dev_idFieldID = (*env)->GetFieldID(env, devInfoCls, "dev_id", "I");
	jfieldID dev_typeFieldID = (*env)->GetFieldID(env, devInfoCls, "ck_type",
			"[B");
	jfieldID dev_valFieldID = (*env)->GetFieldID(env, devInfoCls, "ck_val",
			"[F");
	...
	jfieldID minuteFieldID = (*env)->GetFieldID(env, alarmTimeCls, "minute",
			"I");
	jfieldID secondFieldID = (*env)->GetFieldID(env, alarmTimeCls, "second",
			"I");
...
jint Java_com_topvs_client_NetplayerAPI_GetAvcodecVersion(JNIEnv *env,
		jobject obj) {
	return getAvcodecVersion();
}

//2014-5-29 add_by linzh107
jint Java_com_topvs_client_NetplayerAPI_GetAudioParam(JNIEnv *env, jobject obj,
		jintArray pOutArray) {
...
//		LOGE("GetAudioFrame pAudioFrame->data == NULL");
	...
	int iPackNum = (int) (pAudioData[0] | pAudioData[1] << 8
			| pAudioData[2] << 16 | pAudioData[3] << 24);
	LOGI("GetAudioParam iPackNum = %d", iPackNum);
	int iPackLen = (int) (pAudioData[4] | pAudioData[5] << 8
			| pAudioData[6] << 16 | pAudioData[7] << 24);
...
jshortArray Java_com_topvs_client_NetplayerAPI_GetAudioFrame(JNIEnv *env,
		jobject obj, jint frame_cnt) {
	Hi3510_frame_t *pAudioFrame = st_getFrameListHead(AUDIO_FRAME);
 ...
 * 结果：已确定，废弃该 function
 * */
jbyteArray Java_com_topvs_client_NetplayerAPI_GetAudioFrame2(JNIEnv *env,
		jobject obj, jint frame_cnt) {
	Hi3510_frame_t *pAudioFrame = st_getFrameListHead(AUDIO_FRAME);
	if (pAudioFrame == NULL || pAudioFrame->data == NULL) {
		return NULL;
...
jint Java_com_topvs_client_NetplayerAPI_startDualTalk(JNIEnv* env,
		jclass object, jint user_id) {
	int ret = st_net_registerTalkStreamCallback(user_id, dupRecv_callback,
			NULL);
	if (ret == 0) {
...
jint Java_com_topvs_client_NetplayerAPI_stopDualTalk(JNIEnv* env, jclass object,
		jint user_id) {
	return st_net_stopDualTalk(user_id);
}

jint Java_com_topvs_client_NetplayerAPI_sendVoiceData(JNIEnv *env, jobject obj,
		jint user_id, jshortArray VoiceArray, jint dataLen) {
	...
	frameno++;
	pframe_head.video_resolution = (audio_talk_format & 0x03)
			| (audio_amr_mode << 2);
	...
	ret = st_net_sendAudioData(user_id, sendbuf,
			sizeof(frame_head_t) + RealLength);
